...
async def discover_by_genre(genre_id, lang="fr"):
    url = f"{BASE_URL}/discover/movie"
    params = {"api_key": TMDB_KEY, "language": LANG_MAP.get(lang, "en-US"), "with_genres": genre_id, "sort_by": "popularity.desc"}
    async with httpx.AsyncClient() as client:
        r = await client.get(url, params=params)
        return r.json().get("results", [])[:10]

async def get_trending(lang="fr"):
