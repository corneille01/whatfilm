...
# ── Session store (simple dict avec expiration) ──────────────────────
...
class ContinueRequest(BaseModel):
    session_id: str
    ocr_text: str = ""       # Peut être vide si l'OCR a été fait côté serveur
    transcript: str
...
async def analyser(req: VideoRequest):
    ...
    try:
        ...
        print("STEP 5: TRANSCRIPTION (Groq)")
        transcript = ""
        if audio_exists:
            try:
        ...
        # ── Fallback local : envoyer les données au client ─────────────
        if use_local_fallback:
            print("STEP 5b: Envoi frames+audio au client pour OCR+transcription locale")
        ...
        if not candidates and req.lang != "en":
        ...
        if not result or not result.get("id"):
            best = candidates[0]
            result = {"meilleur_titre": best.get("title") or best.get("name") or "Inconnu", "id": best.get("id"), "score": 35}
        ...
        # ── 10. Détails TMDB ───────────────────────────────────────────
        print(f"STEP 10: TMDB DETAILS for id={movie_id} type={search_type}")
        details = {}
        if movie_id:
            try:
                if search_type == "tv":
        ...
        # Déterminer la région pour les fournisseurs de streaming
        ...
        trailer_data = next(
            ...
            (v for v in details.get("videos", {}).get("results", [])
             if v.get("type") in ["Teaser", "Clip", "Featurette"]), None
        )
        trailer_url = ""
        if trailer_data and trailer_data.get("site") == "YouTube":
            trailer_url = f"https://www.youtube.com/watch?v={trailer_data['key']}"
        ...
        # Ne pas supprimer si on attend une transcription locale
...
async def analyser_continue(req: ContinueRequest):
    ...
    try:
        ...
        # Extraction Gemini
        extraction = await multimodal_extract(frames_paths, ocr_text, transcript)
        if not extraction:
            extraction = {}
        ...
        for q in queries:
            ...
            if results:
                candidates = results; used_query = q; search_type = "movie"; break
        ...
        if not candidates:
            for q in queries:
                try:
                    ...
                    if results:
                        candidates = results; used_query = q; search_type = "tv"; break
                except Exception:
                    pass
        ...
        if not candidates:
            ...
            return {
                "status": "unknown",
                "message": f"Film/Série introuvable. Gemini a identifié : \"{desc[:120]}\""
            }
        ...
        if confidence < 30:
            ...
            return {
                ...
                "description": desc,
                "titre_gemini": titre_gemini,
                "search_youtube": f"https://www.youtube.com/results?search_query={urllib.parse.quote(titre_gemini + ' film')}",
                "search_google": f"https://www.google.com/search?q={urllib.parse.quote(titre_gemini + ' film')}",
                "search_tmdb": f"https://www.themoviedb.org/search?query={urllib.parse.quote(titre_gemini)}",
            }
