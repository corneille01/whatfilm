...
# Max 3 analyses simultanées — évite les 502 sur Render free tier
...
# ANALYSE PRINCIPALE
# Flux :
#   1. yt-dlp télécharge la vidéo
#   2. ffmpeg extrait l'audio
#   3. extract_keyframes() → frames JPG
#   4. OCR serveur → toujours "" (ocr_engine allégé) → ocr_text = ""
#   5. Groq API transcrit l'audio → transcript
#      • Groq OK  → process_analysis() directement (chemin rapide)
#      • Groq KO  → retourne transcription_needed + frames_b64 + audio_b64
#                   → le navigateur fait Tesseract.js + Whisper.js
#                   → POST /analyser_continue avec les résultats
# ════════════════════════════════════════════════════════════════
@app.post("/analyser")
async def analyser(req: VideoRequest):
    ...
    async with _analysis_semaphore:
        ...
        try:
            ...
            # ── STEP 4 : OCR serveur (toujours vide → allégé) ─────────────
            print("🔍 OCR", flush=True)
            ocr_text = extract_text_from_images(frames, max_images=6)
            # ocr_text == "" → normal, pas une erreur

            # ── STEP 5 : Transcription Groq ───────────────────────────────
            print("🎙️ GROQ", flush=True)
            transcript = ""
            if audio_exists:
                try:
                ...
                print("⚠️ Pas d'audio → Groq ignoré, fallback client", flush=True)

            # ── STEP 6 : Décision ─────────────────────────────────────────
            # Groq OK  → analyse directe, pas de fallback navigateur
            # Groq KO ou pas d'audio → fallback navigateur
            #   Le navigateur reçoit les frames (Tesseract.js) + audio si dispo (Whisper.js)
            #   Même sans audio, Tesseract.js peut lire le texte dans les frames
...
# ── ANALYSE CONTINUE (résultats du navigateur) ────────────────────
...
async def analyser_continue(req: ContinueRequest):
    ...
    try:
        ...
        return {"status": "error", "code": "unexpected", "message": "Erreur lors de l'analyse. Réessayez."}
    finally:
